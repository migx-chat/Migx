...
const consumeForGame = async (userId, gameType, amount, gameSessionId = null) => {
  ...
  try {
    ...
    for (const tag of tagsResult.rows) {
      ...
      const merchantCommission = Math.floor(consumeAmount * COMMISSION_RATE / 2);
      const userCommission = Math.floor(consumeAmount * COMMISSION_RATE / 2);
